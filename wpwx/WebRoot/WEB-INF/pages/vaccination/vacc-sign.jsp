        ...
        jQuery(function () {
            ...
            jQuery(document).on("click", "#mySave", null, function () {
                ...
                $.ajax({
                    ...
                    data:{'id':"${VacChildRemind.id}",'vaccId':"${VacChildRemind.vaccId}",'localcode':"${VacChildRemind.localcode}",'signatureDataTo':dataUrl},
                    type:"post",
                    success:function(data){
                        if(data.success){
                            layer.msg(data.msg,{icon: 1,time: 800});
                            //保存成功 跳转到保险购买页面
                            //window.location.href="${ctx}vac/insList.do";
                            setTimeout(function reflush(){window.location.href="${ctx}vac/vaccList.do";},1500);
                        }else{
